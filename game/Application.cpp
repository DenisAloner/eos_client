...
Application::Application()
    ...
    , m_action_animation_timer(new Timer(24, 100))
{
    m_turn = false;
}
...
void Application::render()
{
    ...
    if (m_mouse->m_show_cursor) {
        ...
        rectangle_t<int> rect(mouse.x, mouse.y, 48, 48);
        m_graph->draw_sprite(rect);
...
void Application::initialize(const dimension_t<int> work_area_size, const HDC m_hDC, const HGLRC hRC)
{
    this->m_hDC = m_hDC;
    this->m_hRC = hRC;
...
void Application::update_after_load()
{
    ...
    m_gui_controller.m_GUI->map_viewer->update();
...
void Application::update()
{
    while (true) {
        ...
        m_update_mutex.lock();
        if (m_action_manager->m_is_remove) {
