...
bool GraphicalController::add_scissor(const rectangle_t<float>& rect)
{
    ...
    if (local_scissor.w > global_scissor.w)
        local_scissor.w = global_scissor.w;
    if (local_scissor.y < global_scissor.y)
        local_scissor.y = global_scissor.y;
...
position_t<int> glhUnProjectf(float winx, float winy, float winz, GLdouble* modelview, GLdouble* projection, int* viewport)
{
    ...
    return position_t<int>(out[0] * out[3], out[1] * out[3]);
}

position_t<int> GraphicalController::get_open_gl_position(float x, float y)
{
    GLint viewport[4];
...
GLint GraphicalController::create_empty_texture(dimension_t<int> size)
{
    GLuint texture;
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);
...
void GraphicalController::draw_sprite(const rectangle_t<int>& rect)
{
    glBegin(GL_QUADS);
    glTexCoord2d(0, 0);
...
void GraphicalController::draw_rectangle(const rectangle_t<int>& rect)
{
...
void GraphicalController::draw_tile(tile_t& tile, const rectangle_t<int>& rect)
{
    glBegin(GL_QUADS);
    glTexCoord2d(tile.coordinates[0], tile.coordinates[1]);
    glVertex2d(rect.x, rect.bottom());
    glTexCoord2d(tile.coordinates[0], tile.coordinates[3]);
    glVertex2d(rect.x, rect.y);
    glTexCoord2d(tile.coordinates[2], tile.coordinates[3]);
    glVertex2d(rect.right(), rect.y);
    glTexCoord2d(tile.coordinates[2], tile.coordinates[1]);
    glVertex2d(rect.right(), rect.bottom());
    glEnd();
}

void GraphicalController::draw_sprite_fbo(double tex_width, double tex_height, const rectangle_t<int>& rect)
{
    glBegin(GL_QUADS);
...
void GraphicalController::draw_tile_fbo(double tx1, double ty1, double tx2, double ty2, const rectangle_t<int>& rect)
{
    glBegin(GL_QUADS);
