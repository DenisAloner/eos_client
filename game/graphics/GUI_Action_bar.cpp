...
GUI_Action_bar::GUI_Action_bar(int x, int y, int width, int height)
...
void GUI_Action_bar::update_binding()
{
    ...
    for (auto& item : info) {
        auto action_button = new GUI_ActionButton(m_size.w - 4, 40);
        action_button->m_value = item;
        add_item_control(action_button);
    }
}