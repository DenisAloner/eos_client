...
mapviewer_hint_path::mapviewer_hint_path(std::vector<MapCell*>* path, GameObject* object)
    : m_object(object)
    , m_path(path) {};
mapviewer_hint_area::mapviewer_hint_area(GameObject* object, bool consider_object_size)
    : m_consider_object_size(consider_object_size)
    , m_object(object)
{
}
mapviewer_hint_object_area::mapviewer_hint_object_area(GameObject* object)
    : m_object(object)
{
}
mapviewer_hint_line::mapviewer_hint_line(MapCell* cell, GameObject* object)
    : m_cell(cell)
    , m_object(object)
{
}
...
void gui_mapviewer_hint::draw_cell(GUI_MapViewer* owner, MapCell* cell, int index)
{
    ...
    const rectangle_t<int> r((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp - cell->z * 2) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
    ...
    Application::instance().m_graph->center_text(r.x + r.w * 0.5, r.y + r.h * 0.5, cp1251_to_utf16(std::to_string(index)), 8, 17);
...
void mapviewer_hint_object_area::render(GUI_MapViewer* owner)
{
    ...
    for (int i = 0; i < by; i++) {
        for (int j = 0; j < bx; j++) {
            ...
            rectangle_t<int> r((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
...
void mapviewer_hint_line::draw_cell(GUI_MapViewer* owner, MapCell* a)
{
    ...
    const auto yp = owner->m_tile_count_x - x;
    const auto xp = owner->m_tile_count_y - y;
    const rectangle_t<int> r((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
    glBindTexture(GL_TEXTURE_2D, Application::instance().m_graph->m_select);
    glColor4f(1.0F, 0.9F, 0.0F, 0.5F);
    Application::instance().m_graph->draw_sprite(r);
    glColor4f(1.0F, 0.9F, 0.0F, 1.0F);
    Application::instance().m_graph->center_text(r.x + r.w * 0.5, r.y + r.h * 0.5, cp1251_to_utf16(std::to_string(m_step_count)), 8, 17);
    m_step_count += 1;
}
...
mapviewer_hint_weapon_range::mapviewer_hint_weapon_range(GameObject* object, int range)
    : m_object(object)
    , m_range(range)
{
}
    ...
    for (int i = -1; i < by + 1; i++) {
        for (int j = -1; j < bx + 1; j++) {
            if (i == by || j == bx || i == -1 || j == -1) {
                ...
                int yp = owner->m_tile_count_x - x;
                int xp = owner->m_tile_count_y - y;
                rect = rectangle_t<int>((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
                glColor4f(1.0F, 0.9F, 0.0F, 0.25F);
    ...
    for (int i = 0; i < ay; i++) {
        for (int j = 0; j < ax; j++) {
            ...
            if ((!m_object->is_own(0, xp, yp)) && (rf * rf >= ((j - xrf) * (j - xrf) + (i - yrf) * (i - yrf)))) {
                ...
                int yp = owner->m_tile_count_x - x;
                int xp = owner->m_tile_count_y - y;
                auto r = rectangle_t<int>((xp - yp) * tile_size_x_half + owner->m_shift.x,
                    (xp + yp) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
                glColor4f(1.0F, 0.9F, 0.0F, 0.25F);
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, Application::instance().m_graph->m_empty_01, 0);
                Application::instance().m_graph->draw_sprite(r);
                glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, Application::instance().m_graph->m_empty_03, 0);
...
mapviewer_hint_shoot::mapviewer_hint_shoot(GameObject* object, int range)
    : m_object(object)
    , m_range(range)
{
    m_top = true;
...
mapviewer_object_move::mapviewer_object_move(GameObject* object)
    : m_object(object)
{
}
    ...
    if (m_cell) {
        ...
        for (int i = 0; i < by; i++) {
            for (int j = 0; j < bx; j++) {
                ...
                int yp = owner->m_tile_count_x - x;
                int xp = owner->m_tile_count_y - y;
                r = rectangle_t<int>((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp) * tile_size_y_half + owner->m_shift.y, tile_size_x, -tile_size_y);
                if (valid) {
...
void mapviewer_object_move::render_on_cell(GUI_MapViewer* owner, MapCell* c)
{
    if (m_cell) {
        if (m_cell == c) {
            ...
            int yp = owner->m_tile_count_x - x;
            int xp = owner->m_tile_count_y - y;
            rectangle_t<int> rect((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp + size3d.dx - 1) * tile_size_y_half + owner->m_shift.y, object_size.w, -object_size.h);
            tile_t tile;
            ...
            const auto sprite = tile.unit;
            glUseProgramObjectARB(0);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, sprite);
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, Application::instance().m_graph->m_empty_01, 0);
            Application::instance().m_graph->draw_tile(tile, rect);
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, Application::instance().m_graph->m_empty_03, 0);
            Application::instance().m_graph->draw_tile(tile, rect);
        }
    }
...
mapviewer_object_rotate::mapviewer_object_rotate(GameObject* object)
    : m_object(object)
{
    m_fov = new FOV_help();
}
        ...
        for (int y = m_fov->m_map_center.y - m_fov->m_view.d, yf = m_fov->m_view_center.y - m_fov->m_view.d; y < m_fov->m_map_center.y + m_fov->m_view.u + 1; y++, yf++) {
            for (int x = m_fov->m_map_center.x - m_fov->m_view.l, xf = m_fov->m_view_center.x - m_fov->m_view.l; x < m_fov->m_map_center.x + m_fov->m_view.r + 1; x++, xf++) {
                if (m_fov->m_map[yf][xf].visible) {
                    ...
                    rectangle_t<int> rect(xt, yt, tile_size_x, -tile_size_y);
                    Application::instance().m_graph->draw_sprite(rect);
                    glDisable(GL_BLEND);
                    glDisable(GL_TEXTURE_2D);
                    top_bound = false;
                    if (yf == m_fov->m_view_center.y + m_fov->m_view.u) {
                        ...
                        if (!m_fov->m_map[yf + 1][xf].visible) {
                            top_bound = true;
                        }
                    }
                    ...
                    if (xf == m_fov->m_view_center.x + m_fov->m_view.r) {
                        ...
                        if (!m_fov->m_map[yf][xf + 1].visible) {
                            right_bound = true;
                        }
                    }
...
void mapviewer_object_rotate::render_on_cell(GUI_MapViewer* owner, MapCell* c)
{
    if (owner->m_cursored) {
        if (c == m_object->cell()) {
            ...
            int yp = owner->m_tile_count_x - x;
            int xp = owner->m_tile_count_y - y;
            rectangle_t<int> r((xp - yp) * tile_size_x_half + owner->m_shift.x, (xp + yp + size3d.dx - 1) * tile_size_y_half + owner->m_shift.y, object_size.w, -object_size.h);
            tile_t tile;
...
void GUI_MapViewer::render(GraphicalController* graph, int px, int py)
{
    ...
    if (!m_observer) {
        ...
        fov = ai->m_fov->m_view;
        posc = position_t<int>((m_player->m_object->m_active_state->m_size.dx - 1) >> 1, (m_player->m_object->m_active_state->m_size.dy - 1) >> 1);
    }
    rectangle_t<int> rect;
                ...
                for (int z = 0; z < m_map->m_size.dz; ++z) {
                    ...
                    for (auto current = cell.m_items.begin(); current != cell.m_items.end(); ++current) {
                        ...
                        if (is_draw) {
                            ...
                            rect = rectangle_t<int>((xp - yp) * tile_size_x_half + m_shift.x, (xp + yp) * tile_size_y_half + m_shift.y - z * tile_size_y, object_size.w, -object_size.h);
                            ...
                            vb.vertex[0].position[0] = rect.x;
                            vb.vertex[0].position[1] = rect.bottom();
                            vb.vertex[1].position[0] = rect.x;
                            vb.vertex[1].position[1] = rect.y;
                            vb.vertex[2].position[0] = rect.right();
                            vb.vertex[2].position[1] = rect.y;
                            vb.vertex[3].position[0] = rect.right();
                            vb.vertex[3].position[1] = rect.bottom();
                    ...
                    if (m_z_level == z) {
                        if (m_cursored != nullptr && m_cursored->x == cell.x && m_cursored->y == cell.y) {
                            ...
                            auto yp = m_tile_count_x - px - gx;
                            auto xp = m_tile_count_y - py - gy;
                            rect = rectangle_t<int>((xp - yp) * tile_size_x_half + m_shift.x, (xp + yp) * tile_size_y_half + m_shift.y - z * tile_size_y, tile_size_x, -2 * tile_size_y);
                            auto& vb = m_quads[count];
...
{
    if (e.key == mk_left) {
        const auto p = local_xy(position_t<int>(e.position.x, e.position.y));
...
void GUI_MapViewer::on_mouse_down(MouseEventArgs const& e)
{
    if (e.key == mk_left || e.key == mk_right) {
        position_t<int> p = local_xy(e.position);
        int x;
        int y;
        x = m_center.x + p.x - m_tile_count_x / 2;
        y = m_center.y + p.y - m_tile_count_y / 2;
...
void GUI_MapViewer::on_mouse_wheel(MouseEventArgs const& e)
{
    if (e.value > 0) {
        m_tile_count_x += 2;
        m_tile_count_y += 4;
        if (m_tile_count_x > 64)
            m_tile_count_x = 64;
        if (m_tile_count_y > 128)
            m_tile_count_y = 128;
    } else {
        m_tile_count_x -= 2;
        m_tile_count_y -= 4;
        if (m_tile_count_x < tile_size_x_half)
            m_tile_count_x = tile_size_x_half;
        if (m_tile_count_y < 32)
            m_tile_count_y = 32;
    }
    calculate();
    position_t<int> p = local_xy(position_t<int>(e.position.x, e.position.y));
...
position_t<int> GUI_MapViewer::local_xy(const position_t<int> p)
{
    const auto x = (p.x - m_shift.x - tile_size_x_half) / tile_size_x;
    const auto y = (p.y - m_shift.y) / tile_size_y;
    auto result = position_t<int>(m_tile_count_x - (m_tile_count_y - x - m_tile_count_x + y), m_tile_count_y - (x + y));

    if (result.x > m_tile_count_x - 1) {
        result.x = m_tile_count_x - 1;
    }
    if (result.x < 0) {
        result.x = 0;
    }
    if (result.y > m_tile_count_y - 1) {
        result.y = m_tile_count_y - 1;
    }
    if (result.y < 0) {
        result.y = 0;
    }
