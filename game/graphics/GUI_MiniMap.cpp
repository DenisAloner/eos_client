...
GUI_MiniMap::GUI_MiniMap(position_t<int> position, dimension_t<int> size, GUI_MapViewer* map_viewer)
    : m_map_viewer(map_viewer)
{
    m_position = position;
    m_size = size;
    const auto cr = Application::instance().m_size;
    m_cell_size = dimension_t<float>(static_cast<float>(cr.w) / static_cast<float>(m_map_viewer->m_map->m_size.dx), static_cast<float>(cr.h) / static_cast<float>(m_map_viewer->m_map->m_size.dy));
...
void GUI_MiniMap::render_on_canvas()
{
    if (!m_canvas_create) {
	    const auto cr = Application::instance().m_size;
        m_canvas = Application::instance().m_graph->create_empty_texture(dimension_t<int>(cr.w, cr.h));
    ...
    const auto fbo = Application::instance().m_graph->m_FBO;
    ...
    for (int y = 0; y < m_map_viewer->m_map->m_size.dy; y++) {
        for (int x = 0; x < m_map_viewer->m_map->m_size.dx; x++) {
            ...
            for (auto& m_item : m_map_viewer->m_map->get(0, y, x).m_items)
            {
                glColor4d(0.0, 0.0, 0.0, 0.0);
                if (m_item->m_name == u"floor") {
                    glColor4d(0.3, 0.3, 0.3, 1.0);
                }
                if (m_item->m_name == u"wall") {
                    glColor4d(0.0, 0.3, 0.5, 1.0);
                }
                if (m_item->m_name == u"elf") {
                    glColor4d(0.0, 0.7, 0.0, 1.0);
                }
                if (m_item->m_name == u"door") {
                    glColor4d(0.3, 0.1, 0.0, 1.0);
                }
                if (m_item->m_name == u"torch") {
                    glColor4d(0.9, 0.8, 0.0, 1.0);
                }
                if (m_item->m_name == u"snake") {
...
GUI_FOV::GUI_FOV(position_t<int> position, dimension_t<int> size, GameObject* object)
{
    m_canvas_create = false;
    m_object = object;
...
void GUI_FOV::render_on_canvas()
{
    ...
    m_radius = m_vision_list->m_max_radius;
    m_cell_size = dimension_t<float>(static_cast<float>(cr.w) / static_cast<float>(m_fov->m_view.l + m_fov->m_view.r + 1), static_cast<float>(cr.h) / static_cast<float>(m_fov->m_view.d + m_fov->m_view.u + 1));
    double x0, y0, x1, y1, x2, y2, x3, y3;
    glDisable(GL_BLEND);
    glDisable(GL_TEXTURE_2D);
    glDisable(GL_SCISSOR_TEST);
