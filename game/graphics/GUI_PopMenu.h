...
public:
    ...
    GUI_PopMenu()
        : GUI_Container(0, 0, 0, 0)
    {
        item_click += std::bind(&GUI_PopMenu::on_item_click, this, std::placeholders::_1);
    }
    ~GUI_PopMenu();
    ...
    {
        for (auto& current : m_items) {
            if (current->check_region(MouseEventArgs(position_t<int>(e.position.x - m_position.x, e.position.y - m_position.y), e.key, e.value))) {
                current->mouse_move(MouseEventArgs(position_t<int>(e.position.x - m_position.x, e.position.y - m_position.y), e.key, e.value));
                return;
            }
        }
    };
