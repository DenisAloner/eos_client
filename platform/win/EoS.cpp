...
void disable_graphics(HWND hWnd, HDC hDC);
BOOL set_client_rect(HWND hWnd, int x, int y);
    ...
    LPSTR lpCmdLine, int iCmdShow)
{
    ...
    const dimension_t<int> window_size(work_area.right, work_area.bottom);
    rectangle_t<int> window_rect(work_area.left, work_area.top, window_size);
...
{
    struct Extractor {
        static position_t<int> position(HWND hWnd, LPARAM lParam)
        {
            POINT mouse_pos;
            mouse_pos.x = LOWORD(lParam);
            mouse_pos.y = HIWORD(lParam);
            return position_t<int>(mouse_pos.x, mouse_pos.y);
        }
